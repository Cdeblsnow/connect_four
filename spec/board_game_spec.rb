...
$stdout = File.open(File::NULL, "w") # avoid clogging the terminal with the puts inside the
# tested methods. Added after testing that everything is working properly.
...
describe BoardGame do # rubocop:disable Metrics/BlockLength
  ...
  context "module game tests" do # rubocop:disable Metrics/BlockLength
  ...
  context "module victory tests" do # rubocop:disable Metrics/BlockLength
  ...
  context "module tokens tests" do
    describe "define_player_token" do
      subject(:player_tokens) { described_class.new }
      before do
        player_tokens.player_init
      end
      let(:list_of_players) { player_tokens.instance_variable_get(:@player_list) }
      it "allows a player to select their token" do
        selected_token = player_tokens.instance_variable_get(:@tokens)[0]
        allow(player_tokens).to receive(:gets).and_return("1\n")

        player_tokens.define_player_token(list_of_players[0])
        expect(list_of_players[0].token).to eq(selected_token)
      end
    end
  end
end
